...
- The example components are found in the `packages/library` folder.
...
<Callout>
  **Note:** The Reusables UI is installed in `packages/library/reusables`.
  ...
  through the [CLI](https://reactnativereusables.com/docs/cli) inside `packages/library`.
</Callout>

## Building your own library

You can start by changing the `packages/library` folder name and replace all the references to it through the codebase.

<Callout>
  - All `package.json` files start with `@native-blocks/...` and you will need to update them with your own library name too.
  - All the imports throughout the codebase use aliases defined in `tsconfig.json` files. You can search by `@/library` and replace it with the new name of the folder.
</Callout>
